...
var kurslar = 0;
if (selectValue == "EURO"){
  kurslar = `EURO TRANSLATE ${money * EURO_TO_UZS} so'm`;
} else if ( selectValue == "USD") {
  kurslar = `USD TRANSLATE ${money * USD_TO_UZS} so'm`;
} else if ( selectValue == "RUBL"){
  kurslar = `RUBL TRANSLATE ${money * RUBL_TO_UZS} so'm`;
}
elResult.textContent = `${kurslar}`;
